...
export default class Search {
  ...
  shouldVisit(pos) {
    ...
    if (cellState !== StateMap.searched && cellState !== StateMap.on_deck && cellState !== StateMap.blocked){
 ...
 bfs = () => {
    ...
    while (pq.length > 0) {
      ...
      for (const newPos of Object.values(neighbours)) {
        if (newPos !== undefined) {
          ...
          add_to_pq({ pos: newPos, path: [...path, newPos], weight: weight+thisWeight});
        }
      }
    }
    return undefined
  ...
  greedy = async ()=> {
    ...
    const add_to_pq = (obj, row, col) => {
      ...
      const m_distance = Math.abs(row - this.targetPos.row) + Math.abs(col - this.targetPos.col)
    ...
    while (pq.length > 0) {
      ...
      for (const newPos of Object.values(neighbours)) {
        if (newPos !== undefined) {
          ...
          add_to_pq({ pos: newPos, path: [...path, newPos], weight: thisWeight}, newPos.row, newPos.col);
        }
      }
    }
    return undefined
